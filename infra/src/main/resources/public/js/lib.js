          ...
          if (obj[p] instanceof Array) {
            for (var i = 0; i < obj[p].length; i++) {
              if (typeof obj[p][i] === 'object') {
                throw new TypeError(
                  "Arrays sent as URIs can't contain objects"
                );
              }
              str.push(
      ...
      this.xhr = $.ajax(params)
        .done(function(e, statusText, xhr) {
          if (typeof that.statusCallbacks.done === 'function') {
            if (
              document.cookie === '' &&
              typeof Http.prototype.events.disconnected === 'function'
            ) {
        ...
        .fail(function(e) {
          ...
          if (typeof that.statusCallbacks['e' + e.status] === 'function') {
            ...
            if (!params.disableNotifications && e.status !== 0) {
              humane.spawn({ addnCls: 'humane-original-error' })(
                lang.translate('e' + e.status)
              );
            }
          }
...
(function() {
  ...
  Collection.prototype = {
        ...
        function(e) {
          ...
          if (!cb) {
            ...
            that.callbacks[eventName] = _.without(
              that.callbacks[eventName],
              _.find(that.callbacks[eventName], function(item) {
    ...
    selection: function() {
      ...
      if (
        !this._selection ||
        this._selection.length !== currentSelection.length
      ) {
    ...
    removeSelection: function() {
      ...
      this.all = _.reject(this.all, function(item) {
  ...
  Model.prototype.sync = function() {
    http().get(http().parseUrl(this.api.get, this)).done(
      function(data) {
  ...
  Model.prototype.create = function() {
    http().postJson(http().parseUrl(this.api.post, this), this).done(
      function(data) {
        this.updateData(data);
      }.bind(this)
    );
  ...
  Model.prototype.makeModel = function(fn, methods, namespace) {
    ...
    var ctr = new Function(
      ...
      'return function ' +
        (fn.name || fn._name) +
        '(data){ Model.call(this, data); fn.call(this, data); }'
    )(fn);
    ...
    if (fn.prototype.api) {
      ...
      if (
        fn.prototype.api.post &&
        fn.prototype.api.put &&
        typeof fn.prototype.save !== 'function'
      ) {
    ...
    for (var method in methods) {
      if (method === 'sync' && typeof methods[method] === 'string') {
        (function() {
          ...
          col[method] = function() {
            http().get(http().parseUrl(path, this.composer)).done(
              function(data) {
                this.load(data);
              }.bind(this)
            );
  ...
  Model.prototype.makePermanent = function(obj, methods) {
    ...
    mine.on('sync', function() {
      colContainer.trigger('sync');
    });
    shared.on('sync', function() {
      colContainer.trigger('sync');
    });
    trash.on('sync', function() {
      colContainer.trigger('sync');
    });
    mixed.on('sync', function() {
      colContainer.trigger('sync');
    });
    ...
    mine.sync = function() {
      http()
        .get('/workspace/documents', {
          filter: 'owner',
          application: applicationPrefix + '-' + pluralizeName(obj),
        })
        .done(function(docs) {
    ...
    obj.prototype.save = function() {
      ...
      var blob = new Blob([JSON.stringify(toJson)], {
    ...
    obj.prototype.open = function() {
      this.opened = true;
      http().get('/workspace/document/' + this._id).done(
        function(content) {
  ...
  Model.prototype.updateData = function(newData, triggerEvent) {
    ...
    for (var property in newData) {
      if (
        ...
        !(this[property] instanceof Collection)
      ) {
...
var quickstart = {
  ...
  save: function(cb) {
    http()
      .putJson('/userbook/preference/quickstart', this.state)
      .done(function() {
        if (typeof cb === 'function') {
          cb();
        }
  ...
  load: function(cb) {
    ...
    this.loading = true;
    http().get('/userbook/preference/quickstart').done(
      function(pref) {
        var preferences;
        if (pref.preference) {
          try {
        ...
        if (
          ...
          !(
            ...
            moment(preferences.assistantTimer).dayOfYear() ===
              moment().dayOfYear() &&
            moment(preferences.assistantTimer).hour() === moment().hour()
          )
        ) {
          http().get(skin.basePath + 'template/assistant/steps.json').done(
            function(steps) {
              this.steps = steps;
...
var workspace = {
  thumbnails:
    'thumbnail=120x120&thumbnail=150x150&thumbnail=100x100&thumbnail=290x290&thumbnail=48x48&thumbnail=82x82&thumbnail=381x381',
    ...
    this.protectedDuplicate = function(callback) {
      ...
      xhr.onload = function(e) {
        if (this.status == 200) {
          ...
          http()
            .postFile(
              '/workspace/document?protected=true&application=media-library&' +
  ...
  Folder: function(data) {
    ...
    this.collection(workspace.Folder, {
      sync: function() {
        this.load(
          _.filter(model.mediaLibrary.myDocuments.folders.list, function(
            folder
          ) {
    ...
    this.collection(workspace.Document, {
      sync: function() {
        http()
          .get('/workspace/documents/' + data.folder, {
            filter: 'owner',
            hierarchical: true,
          })
          .done(
    ...
    this.collection(workspace.Folder, {
      sync: function() {
        if (model.me.workflow.workspace.documents.create) {
          http().get('/workspace/folders/list', { filter: 'owner' }).done(
            function(data) {
              this.list = data;
              this.load(
                _.filter(data, function(folder) {
    ...
    this.collection(workspace.Document, {
      sync: function() {
        if (model.me.workflow.workspace.documents.list) {
          http().get('/workspace/documents', { filter: 'shared' }).done(
            function(documents) {
              this.load(documents);
            }.bind(this)
          );
  ...
  AppDocuments: function() {
    this.collection(workspace.Document, {
      sync: function() {
        http().get('/workspace/documents', { filter: 'protected' }).done(
          function(documents) {
            this.load(
...
var Behaviours = (function() {
  return {
    copyRights: function(params) {
      ...
      http().get('/' + infraPrefix + '/public/json/sharing-rights.json').done(
        function(config) {
          http()
            .get('/' + params.provider.application + '/rights/sharing')
            .done(function(providerSharing) {
              http()
                .get('/' + params.target.application + '/rights/sharing')
                .done(function(targetSharing) {
                    ...
                    for (var property in share) {
                      for (var bundle in providerSharing) {
                        if (providerSharing[bundle].indexOf(property) !== -1) {
                          ...
                          config[
                            bundleSplit[bundleSplit.length - 1]
                    ...
                    function addRights(targetResource) {
                      ...
                      for (var bundle in bundles) {
                        ...
                        for (var targetBundle in targetSharing) {
                          ...
                          if (
                            targetBundleSplit[
                              targetBundleSplit.length - 1
                            ].indexOf(bundle) !== -1
                          ) {
                            targetSharing[targetBundle].forEach(function(
                              right
                            ) {
    ...
    findRights: function(serviceName, resource) {
      if (this.applicationsBehaviours[serviceName]) {
        ...
        if (
          typeof this.applicationsBehaviours[serviceName].resource ===
          'function'
        ) {
          console.log(
            'resource method in behaviours is deprecated, please use rights object or rename to resourceRights'
          );
          this.applicationsBehaviours[
            serviceName
          ].resourceRights = this.applicationsBehaviours[serviceName].resource;
        }

        if (
          typeof this.applicationsBehaviours[serviceName].resourceRights !==
            'function' &&
          typeof this.applicationsBehaviours[serviceName].rights === 'object'
        ) {
          var resourceRights = this.applicationsBehaviours[serviceName].rights
            .resource;

          this.applicationsBehaviours[serviceName].resourceRights = function(
            element
          ) {
            for (var behaviour in resourceRights) {
              if (
                ...
                (model.me.hasRight(element, resourceRights[behaviour]) ||
                  (element.owner &&
                    (model.me.userId === element.owner.userId ||
                      model.me.userId === element.owner)))
              ) {
        ...
        if (
          ...
          return this.applicationsBehaviours[serviceName].resourceRights(
            resource
          );
        } else {
          return {};
    ...
    findWorkflow: function(serviceName) {
      var returnWorkflows = function() {
        ...
        if (
          typeof this.applicationsBehaviours[serviceName].workflow ===
          'function'
        ) {
          return this.applicationsBehaviours[serviceName].workflow();
        } else {
          if (
            typeof this.applicationsBehaviours[serviceName].rights ===
              'object' &&
      ...
      for (var prop in obj) {
        if (model.me.hasWorkflow(obj[prop])) {
          ...
          if (typeof dependencies[prop] === 'string') {
            workflow[prop] =
              workflow[prop] && model.me.hasWorkflow(dependencies[prop]);
          }
        }
      }
    ...
    this.collection(calendar.ScheduleItem, {
      ...
      scheduleItemWidth: function(scheduleItem) {
        var concurrentItems = this.filter(function(item) {
          return (
            item.beginning.unix() < scheduleItem.end.unix() &&
            item.end.unix() > scheduleItem.beginning.unix()
          );
        });
        var maxGutter = 0;
      ...
      if (model.me) {
        apps = model.me.apps.filter(function(app) {
          return _.find(apps, function(match) {
            return (
              app.address.indexOf(match) !== -1 && app.icon.indexOf('/') === -1
            );
          });
        });
        apps.push({
      ...
      apps.forEach(function(app) {
        ...
        Behaviours.loadBehaviours(appPrefix, function(behaviours) {
          if (behaviours.sniplets) {
            sniplets.sniplets = sniplets.sniplets.concat(
              _.map(behaviours.sniplets, function(sniplet, name) {
                return {
  ...
  http()
    ...
    .done(function(data) {
      ...
      model.me.hasWorkflow = function(workflow) {
        return (
          _.find(model.me.authorizedActions, function(workflowRight) {
      ...
      model.me.hasRight = function(resource, right) {
        ...
        var currentSharedRights = _.filter(resource.shared, function(
          sharedRight
        ) {
          return (
            model.me.groupsIds.indexOf(sharedRight.groupId) !== -1 ||
            sharedRight.userId === model.me.userId
          );
        });

        var resourceRight =
          _.find(currentSharedRights, function(resourceRight) {
